...
const searchTiket = document.querySelector("#searchtiket");
searchTiket.onkeydown = (event) => {
    if (event.key == "Enter") {
        let data = (searchTiket.value).toUpperCase()
        window.location = `/pesantiket/search/${data}`
    }
}
...
let filter = () => {
    ...
    function setLeftValue() {
        ...
        priceFrom.textContent = `Rp ${_this.value * 1000}`;
    ...
    function setRightValue() {
        ...
        priceTo.textContent = `Rp ${_this.value * 1000}`;
